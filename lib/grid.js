(function (jQuery, methodName, factory) {
    ...
    var _createDOMColumns = function () {
        ...
        $.each(self.columns.left, function (i, col) {
            ...
            $th.hover(function () {
                self.$container.find('[data-colindex=' + col.index + ']').addClass('col-hover');
            }, function () {
                self.$container.find('[data-colindex=' + col.index + ']').removeClass('col-hover');
            });
            $th.appendTo($tr);
        });
    ...
    var _appendRow = function (rowIndex, row) {
        ...
        $.each(this.tables, function (t, table) {
            ...
            $.each(self.columns[table.pos], function (i, col) {
                var rowContent;
                if (typeof self.formatters[col.key] == 'function') {
                    rowContent = self.formatters[col.key](row, col, rowIndex);
                } else {
                ...
                $row.append( $('<td style="width:' + col.width + 'px" data-rowindex="' + rowIndex + '" data-colindex="' + col.index + '"></td>')
                    ...
                    .hover(function () {
                        self.$container.find('[data-rowindex=' + rowIndex + ']').addClass('row-hover');
                        self.$container.find('[data-colindex=' + col.index + ']').addClass('col-hover');
                    }, function () {
                        self.$container.find('[data-rowindex=' + rowIndex + ']').removeClass('row-hover');
                        self.$container.find('[data-colindex=' + col.index + ']').removeClass('col-hover');
                    })
                    .click(function () {
