(function (jQuery, methodName, factory) {
    ...
    var _initColumns = function (colOpts) {
        ...
        if ( (leftColumnsWidth + rightColumnsWidth + mainColumnsWidth + SCROLL_WIDTH + TABLE_BORDER_WIDTH) < this.options.width ) {
            adaptFactor = (this.options.width - leftColumnsWidth - rightColumnsWidth - SCROLL_WIDTH - TABLE_BORDER_WIDTH) / mainColumnsWidth;
            $.each(cols.normal, function (i, c) {
                c.adaptedWidth = c.width * adaptFactor;
            });
        }
