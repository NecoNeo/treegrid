    ...
    var _initDOM = function () {
        ...
        leftPadding = (function () {
            ...
            for (var i = 0; i < self.columns.left.length; i++) {
                sum += (self.columns.left[i].width + 2 * TDTH_PADDING_HOR + TD_BORDER_WIDTH);
            }
            return sum + TD_BORDER_WIDTH;
        })();
        rightPadding = (function () {
            var sum = 0;
            for (var i = 0; i < self.columns.right.length; i++) {
                sum += (self.columns.right[i].width + 2 * TDTH_PADDING_HOR + TD_BORDER_WIDTH);
            }
            return sum + TD_BORDER_WIDTH;
        })();
        topPadding = TH_HEIGHT + 2 * TDTH_PADDING_VER + TABLE_BORDER_WIDTH;
