(function (jQuery, methodName, factory) {
    ...
    var Grid = function (el, options) {
        ...
        this.pixelsPerEm = 16; //refers to how much pixels per em
        this.tables = [];
        this.formatters = {};
        this.sortBy = {};
    ...
    //resolution for css layout using em
    var _initScale = function () {
        var $div = $('<div style="width:1000em;height:0;"></div>');
        $div.appendTo(this.$container);
        this.pixelsPerEm = $div.width() / 1000;
    };

    var _convertIntoPixels = function (str) {
        var pxNum,
            string = String(str),
            reg = /^\d+(\.\d*)?em$/g;
        if (string.match(reg)) {
            pxNum = parseInt(string) * this.pixelsPerEm;
            console.log(pxNum);
        } else {
            pxNum = parseInt(string);
        }
        if (isNaN(pxNum)) {
            pxNum = 150;
        }
    ...
    var _initColumns = function (colOpts) {
        ...
        $.each(colOpts, function (i, opt) {
            ...
            $.extend(col, opt);
            if (col.freeze == 'left') {
                col.adaptedWidth = _convertIntoPixels.call(self, col.width);
                leftColumnsWidth += col.adaptedWidth;
                cols.left.push(col);
            } else if (col.freeze == 'right') {
                col.adaptedWidth = _convertIntoPixels.call(self, col.width);
                rightColumnsWidth += col.adaptedWidth;
                cols.right.push(col);
            } else {
                col.adaptedWidth = _convertIntoPixels.call(self, col.width);
                mainColumnsWidth += col.adaptedWidth;
    ...
    var _DOMResize = function () {
        ...
        this.options.width = _convertIntoPixels.call(this, this.options.width);
        this.options.height = _convertIntoPixels.call(this, this.options.height);
